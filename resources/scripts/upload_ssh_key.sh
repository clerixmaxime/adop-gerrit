...
if curl -sL -w "%{http_code}\\n" "http://localhost:8080/accounts/${user}" -o /dev/null | grep "404" &> /dev/null; then
...
stored_key=$(echo -e $(curl -u ${username}:${password} --silent http://localhost:8080/a/accounts/self/sshkeys | grep "ssh_public_key" | awk '{split($0, a, ": "); print a[2]}' | sed 's/[",]//g'))
...
curl -X POST -u "${username}:${password}" -d "${ssh_key}" "http://localhost:8080/a/accounts/${user}/sshkeys"
